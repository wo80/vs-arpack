 ...
 */
    ...
    if (n > 256) {
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, " ERROR with _NDRV3: N is greater than MAXN ", (ftnlen)43);
	e_wsle();
	goto L9000;
    } else if (nev > 10) {
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, " ERROR with _NDRV3: NEV is greater than MAXNEV ", (ftnlen)47);
	e_wsle();
	goto L9000;
    } else if (ncv > 25) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, " ERROR with _NDRV3: NCV is greater than MAXNCV ", (ftnlen)47);
	e_wsle();
	goto L9000;
    }
    *(unsigned char *)bmat = 'G';
    ...
    for (j = 1; j <= i__1; ++j) {
	i__2 = j - 1;
	q__1.r = h__.r * 1.f - h__.i * 0.f, q__1.i = h__.i * 1.f + h__.r * 
		0.f;
	dl[i__2].r = q__1.r, dl[i__2].i = q__1.i;
	i__2 = j - 1;
	q__1.r = h__.r * 4.f - h__.i * 0.f, q__1.i = h__.r * 0.f + h__.i * 
		4.f;
