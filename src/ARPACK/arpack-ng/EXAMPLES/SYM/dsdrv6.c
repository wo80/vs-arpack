 ...
 */
    ...
    if (n > 256) {
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, " ERROR with _SDRV6: N is greater than MAXN ", (ftnlen)43);
	e_wsle();
	goto L9000;
    } else if (nev > 10) {
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, " ERROR with _SDRV6: NEV is greater than MAXNEV ", (ftnlen)47);
	e_wsle();
	goto L9000;
    } else if (ncv > 25) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, " ERROR with _SDRV6: NCV is greater than MAXNCV ", (ftnlen)47);
	e_wsle();
	goto L9000;
    }
    *bmat = 'G';
...
L10:
	...
	e_wsle();
	s_wsle(&io___45);
	do_lio(&c__9, &c__1, " Check the documentation of _saupd. ", (ftnlen)36);
	e_wsle();
	s_wsle(&io___46);
	do_lio(&c__9, &c__1, " ", (ftnlen)1);
	e_wsle();
...
int dsdrv6_av_(int32_t *n, double *v, double *w)
{
    /* System generated locals */
    int32_t i__1;
    double d__1;
