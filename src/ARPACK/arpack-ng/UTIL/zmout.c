/* D:\Projekte\ARPACK\arpack-ng\UTIL\zmout.f -- translated by f2c (version 20100827). */
...
int zmout_(int32_t *lout, int32_t *m, int32_t *n, 
	zomplex *a, int32_t *lda, int32_t *idigit, char *ifmt)
{
    /* Initialized data */
...
/* Computing MIN */
		...
		for (i = k1; i <= i__2; ++i) {
		    ...
		    do_fio(&c__1, (char *)&i, (ftnlen)sizeof(int32_t));
		}
		e_wsfe();
		i__2 = *m;
		for (i = 1; i <= i__2; ++i) {
			...
			i__3 = k2;
			for (j = k1; j <= i__3; ++j) {
			    do_fio(&c__2, (char *)&a[i + j * a_dim1], (ftnlen)sizeof(double));
			}
			e_wsfe();
		    } else {
			io___12.ciunit = *lout;
...
/* L80: */
		...
		for (i = 1; i <= i__2; ++i) {
		    ...
		    s_wsfe(&io___20);
		    do_fio(&c__1, (char *)&i, (ftnlen)sizeof(int32_t));
		    do_fio(&c__2, (char *)&a[i + k1 * a_dim1], (ftnlen)sizeof(double));
		    e_wsfe();
/* L90: */
		}
/* L100: */
