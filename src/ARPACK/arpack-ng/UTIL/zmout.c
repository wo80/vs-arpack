...
int zmout_(int32_t *m, int32_t *n, zomplex *a, int32_t *lda, int32_t *idigit, char *ifmt)
{
    /* Initialized data */
...
/* Computing MIN */
		...
		for (i = 1; i <= i__2; ++i) {
			...
			for (j = k1; j <= k2; ++j) {
			    d__1 = a[i + j * a_dim1];
			    printf("(  %10.3f   %10.3f)  ", d__1.r, d__1.i);
			}
		}
	    }
...
/* Computing MIN */
		...
		for (i = 1; i <= i__2; ++i) {
			...
			for (j = k1; j <= k2; ++j) {
			    d__1 = a[i + j * a_dim1];
			    printf("(  %12.5f   %12.5f)  ", d__1.r, d__1.i);
			}
		}
	    }
...
/* Computing MIN */
		...
		for (i = 1; i <= i__2; ++i) {
			...
			for (j = k1; j <= k2; ++j) {
			    d__1 = a[i + j * a_dim1];
			    printf("(  %14.7f   %14.7f)  ", d__1.r, d__1.i);
			}
		}
	    }
...
/* Computing MIN */
		...
		for (j = k1; j <= k2; ++j) {
		    d__1 = a[i + j * a_dim1];
		    printf("  %20.13f   %20.13f)  ", d__1.r, d__1.i);
		}
		}
	    }
	}
