/* arpack-ng\SRC\dnaitr.f -- translated by f2c (version 20100827). */
...
L60:
    ...
    dgemv_("T", n, &j, &d_one, &v[v_offset], ldv, &workd[ipj], &i_one, &d_zero, &h[j * h_dim + 1], &i_one);
    ...
    dgemv_("N", n, &j, &d_m1, &v[v_offset], ldv, &h[j * h_dim + 1], &i_one,&d_one, resid, &i_one);
...
L80:
    ...
    daxpy_(&j, &d_one, &workd[irj], &i_one, &h[j * h_dim + 1], &i_one);
