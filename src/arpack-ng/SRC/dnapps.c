            ...
            int *ldq, double *workl, double *workd)
{
    ...
    for (jj = 1; jj <= i__1; ++jj)
    {
        ...
        if (msglvl > 2)
        {
            ...
            dvout_(1, &sigmar, debug_1.ndigit, "_napps: The float part of the shift ");
            dvout_(1, &sigmai, debug_1.ndigit, "_napps: The imaginary part of the shift ");
        }
#endif
        ...
        for (i = istart; i <= i__2; ++i)
        {
            ...
            if (abs(d__1) <= max(d__2, smlnum))
            {
                ...
                if (msglvl > 0)
                {
                    ...
                    dvout_(1, &h[i + 1 + i * h_dim], debug_1.ndigit, "_napps: off diagonal element.");
                }
#endif
...
L110:
    ...
    if (msglvl > 1)
    {
        dvout_(1, &q[kplusp + *kev * q_dim], debug_1.ndigit, "_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}");
        dvout_(1, &h[*kev + 1 + *kev * h_dim], debug_1.ndigit, "_napps: betak = e_{kev+1}^T*H*e_{kev}");
