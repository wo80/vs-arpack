...
L65:
    if (*mode == 2)
    {
        ...
        wnorm = sdot_(n, resid, &i_one, &workd[ivj], &i_one);
    ...
    {
        sgemv_("T", n, &j, &s_one, &v[v_offset], ldv, &workd[ivj], &i_one, &s_zero, &workd[irj], &i_one);
    }
...
L100:
    ...
    if (h[j + h_dim] < 0.0f)
    {
        ...
        if (j < *k + *np)
        {
            sscal_(n, &s_m1, &v[(j + 1) * v_dim + 1], &i_one);
        }
        else
        {
            sscal_(n, &s_m1, resid, &i_one);
        }
    }
