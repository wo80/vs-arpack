        ...
        for (i = istart; i <= i__2; ++i)
        {
            ...
            if ((d__1 = h[i__3].r, abs(d__1)) <= max(d__2,smlnum))
            {
                ...
                if (msglvl > 0)
                {
                    ...
                    zvout_(1, &h[i__3], debug_1.ndigit, "_napps: off diagonal element.");
                }
#endif
...
L100:
    ...
    if (msglvl > 1)
    {
        zvout_(1, &q[kplusp + *kev * q_dim], debug_1.ndigit, "_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}");
        zvout_(1, &h[i__1], debug_1.ndigit, "_napps: betak = e_{kev+1}^T*H*e_{kev}");
