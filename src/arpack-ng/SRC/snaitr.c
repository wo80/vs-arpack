...
L80:
    ...
    if (msglvl > 2)
    {
        xtemp[0] = wnorm;
        xtemp[1] = *rnorm;
        svout_(2, xtemp, debug_1.ndigit, "_naitr: re-orthonalization; wnorm and rnorm are");
...
L90:
    ...
    if (msglvl > 0 && iter > 0)
    {
        ...
        if (msglvl > 2)
        {
            xtemp[0] = *rnorm;
            xtemp[1] = rnorm1;
            svout_(2, xtemp, debug_1.ndigit, "_naitr: iterative refinement ; rnorm and rnorm1 are");
        }
    }
#endif
    ...
    if (j > *k + *np)
    {
        ...
        *ido = 99;
        ...
        if (msglvl > 2)
        {
            i__1 = *k + *np;
            i__2 = *k + *np;
            smout_(1, 2, &h[h_offset], *ldh, debug_1.ndigit, "_naitr: Final upper Hessenberg matrix H of order K+NP");
        }
#endif
