            ...
            int *ierr)
{
    ...
    if (*itry == 1)
    {
        ccopy_(n, &workd[*n], &i_one, resid, &i_one);
    }
    if (*bmat == 'G')
    {
        ++timing_1.nbx;
...
L30:
    ...
    i__1 = *j - 1;
    cgemv_("C", n, &i__1, &c_one, v, ldv, workd, &i_one, &c_zero, &workd[*n], &i_one);
    q__1.r = -1.f, q__1.i = -0.0f;
    cgemv_("N", n, &i__1, &q__1, v, ldv, &workd[*n], &i_one, &c_one, resid, &i_one);
