/* arpack-ng\SRC\znaitr.f -- translated by f2c (version 20100827). */
...
L80:
    ...
    if (msglvl > 2)
    {
        rtemp[0] = wnorm;
        rtemp[1] = *rnorm;
        dvout_(&c__2, rtemp, &debug_1.ndigit, "_naitr: re-orthogonalization; wnorm and rnorm are");
        zvout_(&j, &h[j * h_dim + 1], &debug_1.ndigit, "_naitr: j-th column of H");
    }
#endif
...
L90:
    ...
    if (msglvl > 0 && iter > 0)
    {
        ...
        if (msglvl > 2)
        {
            rtemp[0] = *rnorm;
            rtemp[1] = rnorm1;
            dvout_(&c__2, rtemp, &debug_1.ndigit, "_naitr: iterative refinement ; rnorm and rnorm1 are");
        }
    }
#endif
