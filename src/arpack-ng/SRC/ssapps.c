/* arpack-ng\SRC\ssapps.f -- translated by f2c (version 20100827). */
...
L20:
        ...
        {
            big = (r__1 = h[i + (h_dim << 1)], dabs(r__1)) + (r__2 = h[
                        i + 1 + (h_dim << 1)], dabs(r__2));
            if (h[i + 1 + h_dim] <= epsmch * big)
            {
                ...
                if (msglvl > 0)
                {
                    ...
                    svout_(1, &h[i + 1 + h_dim], debug_1.ndigit, "_sapps: the corresponding off diagonal element");
                }
#endif
...
L40:
        ...
        if (istart < iend)
        {
            ...
            a3 = c * h[istart + 1 + h_dim] - s * h[istart + (h_dim << 1)];
            ...
            for (i = istart + 1; i <= i__2; ++i)
            {
                ...
                for (j = 1; j <= i__3; ++j)
                {
                    a1 = c * q[j + i * q_dim] + s * q[j + (i + 1) * q_dim];
        ...
        if (h[iend + h_dim] < 0.0f)
        {
            h[iend + h_dim] = -h[iend + h_dim];
            sscal_(&kplusp, &s_m1, &q[iend * q_dim + 1], &i_one);
        }
...
L90:
    ...
    for (i = itop; i <= i__1; ++i)
    {
        big = (r__1 = h[i + (h_dim << 1)], dabs(r__1)) + (r__2 = h[i
                + 1 + (h_dim << 1)], dabs(r__2));
        if (h[i + 1 + h_dim] <= epsmch * big)
        {
#ifndef NO_TRACE
    ...
    slacpy_("A", n, kev, &v[(*np + 1) * v_dim + 1], ldv, &v[v_offset], ldv);
    ...
    if (h[*kev + 1 + h_dim] > 0.0f)
    {
        saxpy_(n, &h[*kev + 1 + h_dim], &v[(*kev + 1) * v_dim + 1], &i_one,resid, &i_one);
    }

#ifndef NO_TRACE
    if (msglvl > 1)
    {
        svout_(1, &q[kplusp + *kev * q_dim], debug_1.ndigit, "_sapps: sigmak of the updated residual vector");
        svout_(1, &h[*kev + 1 + h_dim], debug_1.ndigit, "_sapps: betak of the updated residual vector");
        svout_(*kev, &h[(h_dim << 1) + 1], debug_1.ndigit, "_sapps: updated main diagonal of H for next iteration");
        if (*kev > 1)
        {
            i__1 = *kev - 1;
            svout_(1, &h[h_dim + 2], debug_1.ndigit, "_sapps: updated sub diagonal of H for next iteration");
        }
    }
#endif
